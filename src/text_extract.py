...
def _extract_pdf_with_ocr(path: Path) -> str:
    """OCR fallback for scanned PDFs when text layer extraction fails."""

    try:
        import fitz  # type: ignore
    except ImportError as exc:  # pragma: no cover - depends on local environment.
        raise TextExtractionError(
            "OCR fallback requires PyMuPDF (fitz). Install it with 'python -m pip install pymupdf'."
        ) from exc
    ...
    try:
        ...
        raise TextExtractionError(
            "OCR fallback requires pytesseract. Install it with 'python -m pip install pytesseract'."
        ) from exc
    ...
    try:
        ...
        raise TextExtractionError(
            "OCR fallback requires Pillow. Install it with 'python -m pip install pillow'."
        ) from exc

    try:
        document = fitz.open(str(path))
    except Exception as exc:
        raise TextExtractionError(f"OCR failed to open PDF: {exc}") from exc
    ...
    try:
        ...
        raise TextExtractionError(f"OCR processing failed: {exc}") from exc
    finally:
        document.close()
...
def _extract_pdf(path: Path) -> str:
    ...
    try:
        ...
        except TextExtractionError as ocr_exc:
            raise TextExtractionError(
                f"PDF parsing failed: {primary_error}; fallback failed: {fallback_exc}; OCR failed: {ocr_exc}"
            ) from primary_error
