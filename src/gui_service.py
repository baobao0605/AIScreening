...
from src.title_abstract.service import TARunSummary, TitleAbstractService
...
class GuiScreeningService:
    ...
    def run_screening(
        ...
        *,
        ...
        on_finished: Callable[[RunSummary | TARunSummary], None] | None = None,
        on_cancelled: Callable[[RunSummary | TARunSummary], None] | None = None,
    ) -> RunSummary | TARunSummary:
        if overrides.screening_mode == "title_abstract":
            return self._run_title_abstract(
                overrides=overrides,
    ...
    def _run_title_abstract(
        ...
        *,
        ...
        on_paper_finished: Callable[[dict[str, str]], None] | None,
        on_paper_error: Callable[[dict[str, str]], None] | None,
        on_finished: Callable[[TARunSummary], None] | None,
        on_cancelled: Callable[[TARunSummary], None] | None,
    ) -> TARunSummary:
        excel_path = Path(overrides.input_excel).expanduser().resolve()
        ta_service = TitleAbstractService(self.base_dir, self.config_path)
        if overrides.run_mode == "auto_start":
            if on_log is not None:
                on_log("Auto Start phase 1/2: retry failed rows...")
            phase1 = ta_service.run(
                ...
                on_log=on_log,
                on_started=on_started,
                on_record_started=on_paper_started,
                on_record_finished=on_paper_finished,
                on_record_error=on_paper_error,
            )
            if cancel_token is not None and cancel_token.is_cancelled():
                if on_cancelled is not None:
                    on_cancelled(phase1)
                return phase1
            if on_log is not None:
                on_log("Auto Start phase 2/2: screen new rows...")
            phase2 = ta_service.run(
                excel_path=excel_path,
                provider_name=overrides.provider_name,
                api_key=overrides.api_key,
                model=overrides.model,
    ...
    def export(self, *, input_dir: str = "", input_excel: str = "", screening_mode: str = "full_text") -> int:
        if screening_mode == "title_abstract":
            excel_path = Path(input_excel).expanduser().resolve()
            ta_service = TitleAbstractService(self.base_dir, self.config_path)
            workspace = ta_service._workspace_from_excel(excel_path)
            ...
            repo = TARepository(connection)
            repo.initialize()
            return TitleAbstractExporter(repo, excel_path=workspace.excel_path, csv_path=workspace.csv_path).export()
        overrides = RuntimeOverrides(
            input_dir=input_dir,
    ...
    def status(self, *, input_dir: str = "", input_excel: str = "", screening_mode: str = "full_text") -> dict[str, int]:
        if screening_mode == "title_abstract":
            excel_path = Path(input_excel).expanduser().resolve()
            ta_service = TitleAbstractService(self.base_dir, self.config_path)
            workspace = ta_service._workspace_from_excel(excel_path)
    ...
    def list_table_rows(
        ...
        *,
        ...
        if screening_mode == "title_abstract":
            ...
            repo = TARepository(connection)
            repo.initialize()
            return repo.table_rows(limit=limit)
        overrides = RuntimeOverrides(
            input_dir=input_dir,
            provider_name=self.base_settings.provider.name,
            api_key="",
    ...
    def project_workspace(self, *, input_dir: str = "", input_excel: str = "", screening_mode: str = "full_text") -> ProjectWorkspace:
        if screening_mode == "title_abstract":
            excel_path = Path(input_excel).expanduser().resolve()
            return TitleAbstractService(self.base_dir, self.config_path)._workspace_from_excel(excel_path)
        return get_project_workspace(input_dir, output_root=self.base_settings.output_dir)

    def clear_project_history(self, *, input_dir: str = "", input_excel: str = "", screening_mode: str = "full_text") -> None:
        if screening_mode == "title_abstract":
            workspace = self.project_workspace(input_excel=input_excel, screening_mode="title_abstract")
