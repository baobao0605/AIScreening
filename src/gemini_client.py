...
def _is_non_retryable_quota_error(exc: Exception) -> bool:
    """Return True when retries are unlikely to help (e.g., depleted credits)."""
...
class GeminiClient:
    ...
    def screen(self, prompt: str) -> str:
        ...
        if last_error is not None:
            if _is_non_retryable_quota_error(last_error):
                raise GeminiClientError(
                    "Gemini billing credits appear depleted. Please top up billing in AI Studio and retry."
                ) from last_error
            raise GeminiClientError(
