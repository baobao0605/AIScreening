...
class ScreeningPipeline:
    ...
    def _process_paper(self, paper: PaperRecord) -> str:
        ...
        try:
            ...
            if paper.file_ext.lower() == ".pdf" and (
                "No extractable text found" in message_text
                or "OCR fallback requires" in message_text
                or "OCR failed" in message_text
            ):
                title = paper.title or paper.file_name
                doi = paper.doi or ""
                note = (
                    "No extractable text from PDF (likely scanned/image-based). "
                    "Marked MAYBE; manual/OCR check recommended."
                )
                self.repository.mark_done(
                    ...
                    construct="unclear",
                    note=note,
                    screening_model="system / ocr-fallback",
                    prompt_version=self.settings.app.prompt_version,
                )
                self.logger.warning("%s | %s", paper.file_name, note)
        ...
        try:
            ...
            final_error = exc
            repaired_raw_response: str | None = None
            try:
                repair_prompt = self._build_repair_prompt(
                    validation_error=str(exc),
                    original_response=raw_response or "",
                )
                repaired_raw_response = self.llm_provider.screen(repair_prompt)
                validated = validate_model_output(repaired_raw_response)
                self.repository.create_screening_run(
                    paper_id=paper.paper_id,
                    model_name=model_name,
                    prompt_version=self.settings.app.prompt_version,
                    raw_response=repaired_raw_response if self.settings.screening.save_raw_response else None,
                    parsed_ok=True,
                )
                payload = validated.to_db_payload()
                self.repository.mark_done(
                ...
                self.logger.info(
                    "%s screened successfully after response repair as %s",
                    paper.file_name,
                    payload["Decision"],
                )
                return PaperStatus.DONE.value
            except Exception as repair_exc:
                raw_response = repaired_raw_response or raw_response
                final_error = repair_exc
        except Exception as exc:
            final_error = exc

        assert final_error is not None
        raw_response = raw_response or getattr(final_error, "raw_response", None)
        self.repository.create_screening_run(
            paper_id=paper.paper_id,
            model_name=model_name,
            prompt_version=self.settings.app.prompt_version,
        ...
        message = f"Screening failed: {final_error}"
    ...
    def _build_repair_prompt(*, validation_error: str, original_response: str) -> str:
        """Ask model to repair JSON into the exact target schema."""

        return (
            "Repair the JSON below.\n"
            "Return JSON only, no markdown.\n"
            "Use exactly these keys: Title, DOI, Decision, Exclude reason, Construct, Note.\n"
            "Decision must be one of INCLUDE/EXCLUDE/MAYBE.\n"
            "Exclude reason must be empty for INCLUDE/MAYBE; for EXCLUDE use one of: "
            "Wrong topic, Qualitative only, Foreign language, Exp is a predictor, "
            "Wrong EXP timing, Wrong publication type, Wrong population, No effect size.\n"
            'Construct must be exactly "target construct" or "unclear".\n'
            "Keep original meaning; minimally edit.\n\n"
            f"Validation error:\n{validation_error}\n\n"
            f"Original response:\n{original_response}\n"
        )
