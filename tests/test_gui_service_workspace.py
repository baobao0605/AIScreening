...
def test_title_abstract_auto_start_runs_retry_then_new_only(monkeypatch, tmp_path: Path) -> None:
    settings_path = _write_settings(tmp_path)
    service = GuiScreeningService(settings_path)
    excel_path = tmp_path / "ta.xlsx"
    excel_path.write_bytes(b"placeholder")
    calls: list[str] = []

    class DummyTAService:
        def __init__(self, *_args, **_kwargs) -> None:
            pass

        def run(self, **kwargs):  # noqa: ANN003
            calls.append(kwargs["run_mode"])
            from src.title_abstract.service import TARunSummary

            return TARunSummary(queued=1, done=1, failed=0, exported_rows=1)

    monkeypatch.setattr("src.gui_service.TitleAbstractService", DummyTAService)
    overrides = RuntimeOverrides(
        input_dir=str(tmp_path / "input" / "unused"),
        ...
        base_url="",
        prompt_text="criteria",
        prompt_path=str(tmp_path / "config" / "criteria_prompt.txt"),
        screening_mode="title_abstract",
        run_mode="auto_start",
    )
    service.run_screening(overrides=overrides)
    assert calls == ["retry_failed", "new_only"]
