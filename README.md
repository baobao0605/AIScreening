...
The GUI now supports two workflows:
- Full-text Screening (PDF/DOCX/TXT folder)
- Title-Abstract Screening (Excel `.xlsx`)

Both workflows reuse the same provider/config foundations and keep local SQLite + export files per project workspace.
...
- Existing CLI and full-text GUI behavior remains unchanged
...
- Current MVP does not include column-mapping UI; if columns are different, adjust Excel first
...
### Full-text projects (per input folder)
...
### Title-Abstract projects (per input Excel)
...
Notes:
- Switching Excel switches current title-abstract project automatically
- Switching back restores previous history
- `Clear Current Project History` only clears current project workspace and does not delete the original Excel
...
## GUI Run Modes (Both Screening Modes)
...
- Title-Abstract: retries failed retryable rows in current Excel project
- Does not rerun `DONE`

Auto Start:
- Phase 1: retry failed papers in current project
- Phase 2: screen `NEW` only
- Designed to avoid phase-2 reprocessing retry-failed items
...
- It means model output did not match the allowed `Exclude reason` list.
- Save a stricter Title-Abstract prompt and click `Retry Failed`.

If Gemini returns `503 UNAVAILABLE` / high-demand errors:
...
Title-Abstract defaults (inside current `ta_...` project workspace):
...
9. Verify `title_abstract_screening_log.xlsx/.csv` are generated in current `ta_...` project.
